...
router.post('/order', async function (req, res, next) {
  ...
  const { name, price, image,quantity,type,email,nameuser,address,phone } = req.body;
 
  const result = await orderController.orderRes( name, price, image,quantity,type,email,nameuser,address,phone );
...
router.post('/updatepassword', async function (req, res, next){
  ...
  const result = await userController.updatePassword({_id:id}, password);
...
router.post('/editprofile', async function (req, res, next) {
  ...
  const result = await userController.update({_id:id}, name, gender, address, phone );
  if (result) {
    res.json({status: true});
  } else {
    res.json({status: false});
...
router.get('/profile/:id/detail', async function (req, res, next) {
  ...
  const {id} = req.params;
  //lay danh sach san pham
  const profile = await userController.getProfileById1(id);
  res.json(profile);
});
...
router.get('/profile/:email', async function (req, res, next) {
  ...
  const user = await userController.getProfileById(email);
  console.log(email);
  res.json(user);
});
